...
time output=`curl -s -H "Content-Type: application/json" -X POST -d  $json https://bfi23l3l42.execute-api.us-east-1.amazonaws.com/hello1`
...
time output=`aws lambda invoke --invocation-type RequestResponse --function-name lambda_hello --region us-east-1 --payload $json /dev/stdout | head -n 1 | head -c -2 ; echo`
